  ...
  <div class="git-monitor">
    <el-card shadow="never">
      <el-table :data="tableData" v-loading="loading" stripe>
        ...
        <el-table-column prop="publish_time" label="时间" width="170" />
      </el-table>
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await intelApi.getGitMonitor({ limit: 10, offset: (currentPage.value - 1) * 10 })
    if (res.status) {
