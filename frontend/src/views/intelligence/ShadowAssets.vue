  ...
  <div class="shadow-assets">
    <el-card shadow="never">
      <el-table :data="tableData" v-loading="loading" stripe>
        ...
        <el-table-column prop="discover_time" label="发现时间" width="170" />
      </el-table>
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await intelApi.getShadowAssets({ limit: 10, offset: (currentPage.value - 1) * 10 })
    if (res.status) {
